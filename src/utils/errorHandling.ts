    ...
    if (apiError.detail && Array.isArray(apiError.detail)) {
      apiError.detail.forEach((error: any) => {
        if (error.loc && error.msg) {
          ...
          const fieldPath = error.loc.filter((loc: any) => loc !== 'body').join('.');
          if (fieldPath) {
            errors[fieldPath] = {
              message: error.msg,
...
export function logFormSubmissionError(formData: any, apiError: any, context: string) {
...
export function getFieldDisplayName(fieldName: string): string {
  return FIELD_DISPLAY_NAMES[fieldName] || fieldName.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
}

// Format validation error message for display
export function formatErrorMessage(fieldName: string, error: { message: string; type: string }): string {
  const displayName = getFieldDisplayName(fieldName);
  
  // Special handling for time fields
  if ((fieldName === 'time_start' || fieldName === 'time_end') && 
      (error.type === 'value_error' || error.type === 'type_error' || error.message.includes('time'))) {
