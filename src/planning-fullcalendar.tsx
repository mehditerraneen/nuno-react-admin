...
const PlanningFCCalendar: React.FC<PlanningFCCalendarProps> = ({
  planningId,
}) => {
  ...
  const getCalendarEvents = useCallback(() => {
    ...
    if (filterEmployees.length > 0) {
      filteredEmployees = employees.filter((emp: any) =>
        filterEmployees.includes(emp.employee_id),
      );
    }
    if (filterJobPositions.length > 0) {
      filteredEmployees = filteredEmployees.filter((emp: any) =>
        filterJobPositions.includes(emp.job_position),
      );
    }
  ...
  const renderEventContent = (arg: EventContentArg) => {
    const { abbreviation, shiftCode, hours, source, isOff } =
      arg.event.extendedProps;

    return (
      <Tooltip
        title={`${arg.event.extendedProps.employeeName} - ${shiftCode} (${hours}h)`}
      >
        <Box
          ...
          sx={{
            ...
            whiteSpace: "nowrap",
            borderLeft:
              source === "OPTIMIZER" ? "2px solid #2196F3" : undefined,
            opacity: isOff ? 0.7 : 1,
  ...
  const getDayCellClassNames = (arg: { date: Date }) => {
    ...
    // Holiday styling
    if (
      calendarData?.luxembourg_holidays?.[day] &&
      month === calendarData.planning?.month &&
      year === calendarData.planning?.year
    ) {
  ...
  const handleClearOptimizerShifts = async () => {
    if (!window.confirm("Effacer tous les shifts générés par l'optimiseur?"))
      return;
  ...
  const uniqueJobPositions = [
    ...new Set(employees.map((e: any) => e.job_position).filter(Boolean)),
  ].sort();
  ...
  return (
    <Box className="planning-fullcalendar">
        ...
        <DialogContent>
          <Box display="flex" flexDirection="column" gap={2} mt={2}>
            ...
            <ToggleButtonGroup
              ...
              exclusive
              onChange={(_, newValue) =>
                newValue && setSelectedAlgorithm(newValue)
              }
              fullWidth
            >
      ...
      >
        ...
        <DialogActions>
          <Button
            onClick={() => setCsvImportDialog(false)}
            disabled={importing}
          >
            Annuler
          </Button>
