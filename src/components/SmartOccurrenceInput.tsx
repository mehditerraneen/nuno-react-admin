...
export const SmartOccurrenceInput: React.FC<SmartOccurrenceInputProps> = ({
  ...
  const handleChange = (newSelectedIds: number[]) => {
    ...
    if (isTousLesJoursSelected && newSelectedIds.length > 1) {
      console.log(
        '📅 "Tous les jours" already selected - ignoring other selections',
      );
  ...
  const tousLesJoursId = tousLesJoursOcc?.id;
  const isTousLesJoursSelected =
    tousLesJoursId && selectedIds.includes(tousLesJoursId);
    ...
    <FormControl component="fieldset" required={required} fullWidth>
      ...
      <FormGroup>
        {occurrences.map((occurrence) => {
          const isDisabled =
            isTousLesJoursSelected && occurrence.id !== tousLesJoursId;
              ...
              label={
                <span
                  style={{
                    ...
                    fontWeight:
                      occurrence.id === tousLesJoursId ? "bold" : "normal",
                  }}
                >
                  {occurrence.str_name}
