...
const PrintContent = React.forwardRef<HTMLDivElement, CarePlanPrintViewProps>(
  ({ record, patient, details }, ref) => {
      ...
      if (record?.medical_care_summary_per_patient_id) {
        ...
        dataProvider
          ...
          .then((cnsDetails: any[]) => {
            setMissingCodes(
              cnsDetails.filter(
                (d: any) => d.item?.code && !usedCodes.has(d.item.code),
              ),
            );
          })
          .catch(() => {});
    ...
    const codeStats = new Map<
      string,
      { weeklyPkg: number; sessionDur: number; totalOcc: number }
    >();
    let totalSessionMinutes = 0;
    details.forEach((detail) => {
      const daysPerWeek =
        calculateActualDaysPerWeek(detail.params_occurrence) || 1;
      const sessionDur = calculateSessionDuration(
        detail.time_start,
        detail.time_end,
      );
      ...
      detail.longtermcareitemquantity_set.forEach((item) => {
        ...
        if (existing) {
          ...
          codeStats.set(code, {
            weeklyPkg: wp,
            sessionDur: sd,
            totalOcc: item.quantity * daysPerWeek,
          });
        }
      });
    });
    const totalWeeklyPkg = Array.from(codeStats.values()).reduce(
      (s, v) => s + v.weeklyPkg,
      0,
    );
    const totalConsumed = Array.from(codeStats.values()).reduce(
      (s, v) => s + Math.round(v.sessionDur * v.totalOcc * 100) / 100,
      0,
    );
    ...
    return (
      <Box ref={ref} sx={{ p: 4, maxWidth: 800, mx: "auto", ...printStyles }}>
        ...
        {(() => {
          ...
          details.forEach((detail) => {
            const isEveryDay = detail.params_occurrence.some(
              (o: CareOccurrence) => o.value === "*",
            );
            const activeDays = isEveryDay
              ? DAYS.map((d) => d.value)
              : detail.params_occurrence
                  .map((o: CareOccurrence) => o.value)
                  .filter((v) => v !== "x");
            const dur = calculateSessionDuration(
              detail.time_start,
              detail.time_end,
            );
          ...
          return (
            <Box sx={{ mb: 3, pageBreakInside: "avoid" }}>
              <Typography
                variant="h6"
                sx={{ fontWeight: 600, mb: 1.5, color: "#1976d2" }}
              >
            ...
            <Box key={detail.id} sx={{ mb: 2.5, pageBreakInside: "avoid" }}>
              ...
              >
                ...
                <Typography sx={{ fontSize: "10pt" }}>
                  {formatTime(detail.time_start)} —{" "}
                  {formatTime(detail.time_end)} ({sessionDur} min)
                </Typography>
              </Box>

              <Box
                sx={{ border: "1px solid #e0e0e0", borderTop: "none", p: 1.5 }}
              >
                {occurrenceText && (
                  <Typography variant="body2" sx={{ mb: 1, color: "#666" }}>
                    <strong>Jours:</strong> {occurrenceText} ({daysPerWeek}
                    x/sem.)
                  </Typography>
                )}
                  ...
                  <TableBody>
                    {detail.longtermcareitemquantity_set.map((item, i) => {
                      ...
                      return (
                        <TableRow key={i}>
                          <TableCell sx={cell}>
                            {item.long_term_care_item.code}
                          </TableCell>
                          <TableCell sx={cell}>
                            {(item as any).custom_description ||
                              item.long_term_care_item.description ||
                              "—"}
                          </TableCell>
                          ...
                          <TableCell align="right" sx={cell}>
                            {Math.round(sd * item.quantity * 100) / 100} min
                          </TableCell>
                        </TableRow>
                      );
                    })}
          ...
          <Table size="small">
            ...
            <TableBody>
                ...
                <TableRow key={code}>
                  ...
                  <TableCell sx={{ ...cell, fontSize: "9pt" }}>
                    {(() => {
                      ...
                      return (
                        (match as any)?.custom_description ||
                        match?.long_term_care_item.description ||
                        "—"
                      );
                    })()}
                  </TableCell>
                  ...
                  <TableCell align="right" sx={{ ...cell, fontWeight: 600 }}>
                    {Math.round(stats.sessionDur * stats.totalOcc * 100) / 100}{" "}
                    min
                  </TableCell>
                  <TableCell align="right" sx={cell}>
                    {stats.weeklyPkg} min
              ...
              <TableRow sx={{ backgroundColor: "#f0f4f8" }}>
                <TableCell colSpan={4} sx={{ ...cell, fontWeight: 700 }}>
                  TOTAL
                </TableCell>
        ...
        {missingCodes.length > 0 && (
          <Box sx={{ pageBreakInside: "avoid", mt: 3 }}>
            <Typography
              variant="h6"
              sx={{ fontWeight: 600, mb: 1, color: "#ed6c02" }}
            >
        ...
        <Box sx={{ display: "flex", justifyContent: "space-between" }}>
          <Typography variant="caption" color="text.secondary">
            {branding?.company_name || ""} — Plan de soins n&deg;
            {record?.plan_number} — {patientName}
          </Typography>
          <Typography variant="caption" color="text.secondary">
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      const response = await authenticatedFetch(
        `${apiUrl}/careplans/${record.id}/pdf`,
      );
