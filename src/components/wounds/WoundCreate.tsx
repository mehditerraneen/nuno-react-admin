...
const WoundCreateForm = () => {
  ...
  const handleConfirm = () => {
    ...
    // Notify user
    notify(
      "Wound location marked. You can now fill in additional details and save.",
      { type: "info" },
    );
  };
  ...
  return (
    <>
      {/* Patient Selection */}
      <ReferenceInput source="patient" reference="patients">
        <AutocompleteInput
      ...
      {tempWoundData && (
        <>
          <TextInput
            source="x_position"
            defaultValue={tempWoundData.x}
            sx={{ display: "none" }}
          />
          <TextInput
            source="y_position"
            defaultValue={tempWoundData.y}
            sx={{ display: "none" }}
          />
          <TextInput
            source="body_view"
            defaultValue={bodyView}
            sx={{ display: "none" }}
          />
          <TextInput
            source="body_area"
            defaultValue={tempWoundData.bodyArea}
            sx={{ display: "none" }}
          />
        </>
      )}
            ...
            <Box>
              ...
              <Paper sx={{ p: 2, mt: 2, backgroundColor: "#f5f5f5" }}>
                ...
                <Typography variant="body2" color="text.secondary">
                  <strong>Coordinates:</strong> ({tempWoundData.x},{" "}
                  {tempWoundData.y})
                </Typography>
              </Paper>
              <Typography variant="body2" color="text.secondary" sx={{ mt: 2 }}>
                Click Confirm to proceed with creating the wound at this
                location. You can add more details after confirmation.
              </Typography>
            </Box>
          )}
        </DialogContent>
