...
export const TabbedCareFormLayout: React.FC<TabbedCareFormLayoutProps> = ({ 
  ...
  const [expandedAccordion, setExpandedAccordion] = useState<string | false>('care-items');
  ...
  const getTabErrors = (tabIndex: number) => {
    ...
    switch (tabIndex) {
      case 0: // Basic Info
        return errorFields.filter(field => ['name', 'care_actions'].includes(field));
      case 1: // Schedule
        return errorFields.filter(field => ['time_start', 'time_end', 'params_occurrence_ids'].includes(field));
      case 2: // Care Items
        return errorFields.filter(field => field.includes('long_term_care_item'));
      default:
        return [];
    }
  };
  ...
  const careItemFilter = useMemo(() => {
    ...
    if (!cnsItemIds || cnsItemIds.length === 0 || (cnsItemIds.length === 1 && cnsItemIds[0] === -1)) {
            ...
            <Step key={index}>
              <StepLabel 
                icon={React.cloneElement(tab.icon, { 
                  color: tab.hasErrors ? 'error' : (activeTab >= index ? tab.color : 'disabled')
                })}
                error={tab.hasErrors}
              >
                <Typography variant="caption" color={tab.hasErrors ? 'error' : 'inherit'}>
                  {tab.label}
                  {tab.hasErrors && ' ⚠️'}
                </Typography>
              </StepLabel>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0.5 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    {React.cloneElement(tab.icon, { 
                      color: tab.hasErrors ? 'error' : (activeTab === index ? tab.color : 'action')
                    })}
                    <Typography variant="subtitle2" color={tab.hasErrors ? 'error' : 'inherit'}>
                      {tab.label}
                    </Typography>
                    {tab.hasErrors && (
        ...
        <CardContent sx={{ p: 3, minHeight: 400 }}>
          ...
          {activeTab === 0 && (
            <Box>
              ...
              <Grid container spacing={3}>
                <Grid item xs={12}>
                  <TextInput 
                    ...
                    fullWidth 
                    required 
                    helperText={validationErrors.name || "Enter a descriptive name for this care plan detail"}
                ...
                <Grid item xs={12}>
                  <TextInput 
                    ...
                    rows={4} 
                    required
                    helperText={validationErrors.care_actions || "Describe the specific care actions to be performed during this session"}
              ...
              {/* Next Step Hint */}
              <Paper variant="outlined" sx={{ p: 2, mt: 3, backgroundColor: '#e3f2fd' }}>
          ...
          {activeTab === 1 && (
            <Box>
                ...
                <Grid container spacing={3}>
                  <Grid item xs={12} md={6}>
                    <SmartTimeInput
                      ...
                      label="Start Time"
                      required
                      helperText={validationErrors.time_start || "When does this care session begin?"}
                  ...
                  <Grid item xs={12} md={6}>
                    <SmartTimeInput 
                      ...
                      helperText={validationErrors.time_end || "End time (auto-suggested based on care items)"}
              ...
              {/* Next Step Hint */}
              <Paper variant="outlined" sx={{ p: 2, mt: 3, backgroundColor: '#e8f5e8' }}>
                <Typography variant="body2" sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Box>
              ...
              >
                ...
                >
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, width: '100%' }}>
                    ...
                    <Chip 
                      label={expandedAccordion === 'care-items' ? 'Collapse' : 'Expand to configure'} 
                      size="small" 
                      variant="outlined"
                      color="success"
                    />
                ...
                <AccordionDetails sx={{ pt: 2 }}>
                  <Box sx={{ width: '100%' }}>
                    ...
                    >
                      <Typography variant="body2" color="text.secondary">
                        💡 <strong>Tips:</strong> Each care item shows its daily duration (weekly package ÷ 7). 
                        The end time will auto-suggest based on total daily duration. Quantity defaults to 1.
                      </Typography>
                    </Paper>
                            ...
                            >
                              <SelectInput
                                optionText={(choice: LongTermCareItem) => {
                                  const dailyDuration = (choice.weekly_package || 0) / 7;
                                  const dailyText = dailyDuration > 0 ? ` (${formatDurationDisplay(dailyDuration)}/day)` : '';
                                  ...
                                  return (
                                    <Tooltip
                                      ...
                                      arrow
                                    >
                                      <span>{choice.code}{dailyText}</span>
                                    </Tooltip>
                                  );
                                }}
                              />
              ...
              <Paper variant="outlined" sx={{ p: 2, backgroundColor: '#f3e5f5' }}>
                ...
                <Grid container spacing={2}>
                  <Grid item xs={12} md={6}>
                    <Box component="ul" sx={{ fontSize: '0.875rem', pl: 2, m: 0 }}>
                      <li>Daily duration = weekly package ÷ 7 days</li>
                      <li>End time auto-suggests based on start time + total daily duration</li>
                    </Box>
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <Box component="ul" sx={{ fontSize: '0.875rem', pl: 2, m: 0 }}>
