    ...
    <Create>
      <SimpleForm>
        ...
        <Alert severity="info" sx={{ mt: 2 }}>
          Après création, utilisez le bouton "Générer planning" pour créer
          automatiquement les affectations.
        </Alert>
      </SimpleForm>
    </Create>
  );
...
export const PlanningEdit = () => {
  const planningId = parseInt(
    window.location.hash.match(/\/(\d+)$/)?.[1] || "0",
  );
    ...
    <Tooltip title={employeeName} placement="top" arrow>
      ...
      >
        {shift ? (
          ...
          >
            <Tooltip
              title={`${shift.shift_code} (${shift.hours || 0}h)${shift.source ? " - " + getSourceInfo(shift.source).label : ""} | Glisser-déposer pour copier | Cliquer pour modifier`}
            >
              <Chip
                label={
                ...
                sx={{
                  ...
                  fontSize: "0.7rem",
                  cursor: "grab",
                  borderLeft: shift.source
                    ? `3px solid ${getSourceInfo(shift.source).borderColor}`
                    : undefined,
                  "&:active": {
                    cursor: "grabbing",
                  },
              ...
              <Table size="small">
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell sx={{ minWidth: 500, width: "50%" }}>
                      Horaire (0h → 24h)
                    </TableCell>
                    <TableCell sx={{ minWidth: 120, width: "20%" }}>
                      Shift
                    </TableCell>
                        ...
                        <TableCell>
                          <Box display="flex" flexDirection="column" gap={0.5}>
                            ...
                            <Box display="flex" gap={0.5} flexWrap="wrap">
                              {emp.hours_exceeded && (
                                <Tooltip
                                  title={`Dépassement de ${emp.hours_over_limit?.toFixed(1)}h sur le maximum mensuel`}
                                >
                                  <Chip
                              ...
                              {emp.consecutive_days_violation && (
                                <Tooltip
                                  title={`${emp.max_consecutive_days} jours consécutifs - Loi: max 6 jours (44h repos)`}
                                >
                                  <Chip
                              ...
                              {emp.evening_to_morning_violation && (
                                <Tooltip
                                  title={`Soir→Matin interdit: ${emp.evening_to_morning_violations?.length} violation(s)`}
                                >
                                  <Chip
                                    label={`🔴 Soir→Matin`}
                                    size="small"
...
const PlanningCalendar = ({ planningId }: { planningId: number }) => {
  ...
  const loadData = async () => {
    try {
      ...
      const apiUrl = import.meta.env.VITE_SIMPLE_REST_URL;
      const calendarResponse = await authenticatedFetch(
  ...
  const updateShiftOptimistically = (
    employeeId: number,
    day: number,
    shiftData: any,
  ) => {
  ...
  const handleClearOptimizerShifts = async () => {
    if (
      !window.confirm(
        "Êtes-vous sûr de vouloir effacer tous les shifts générés par l'optimiseur? Les shifts manuels seront conservés.",
      )
    ) {
      return;
    }
  ...
  const validateShiftData = () => {
    ...
    if (
      shiftTypes.some(
        (st) => st.code.toUpperCase() === newShift.code.toUpperCase(),
      )
    ) {
      setShiftValidationError(
        `Le code "${newShift.code}" existe déjà. Veuillez choisir un code unique.`,
      );
      return false;
    }
    ...
    if (newShift.break_minutes >= totalMinutes) {
      setShiftValidationError(
        "La pause ne peut pas être supérieure ou égale à la durée totale du shift",
      );
      return false;
    }
    ...
    if (totalHours > 24) {
      setShiftValidationError(
        "La durée du shift ne peut pas dépasser 24 heures",
      );
      return false;
    }
  ...
  const handleCsvImport = async () => {
    ...
    try {
      ...
      const isSimpleFormat =
        headers.includes("employee_id") || headers.includes("abbreviation");
      ...
      if (isSimpleFormat) {
        ...
        shifts = lines
          ...
          .filter(
            (row) =>
              (row.employee_id || row.abbreviation) &&
              row.date &&
              row.shift_code,
          );
      } else {
        ...
        for (let rowIdx = 8; rowIdx < Math.min(lines.length, 25); rowIdx++) {
          ...
          if (
            !abbreviation ||
            abbreviation === "" ||
            abbreviation.includes("TOUTES LES CELLULES")
          ) {
            continue;
          }
      ...
      if (result.errors && result.errors.length > 0) {
        ...
        notify(
          `⚠️ ${result.imported_count} shifts importés, ${result.errors.length} erreurs`,
          { type: "warning" },
        );
      } else {
  ...
  const handleApplySuggestions = async (dryRun: boolean = false) => {
    try {
      ...
      const apiUrl = import.meta.env.VITE_SIMPLE_REST_URL;
      const response = await authenticatedFetch(
        `${apiUrl}/planning/monthly-planning/${planningId}/apply-suggestions`,
        {
          method: "POST",
          body: JSON.stringify({
            dry_run: dryRun,
      ...
      if (dryRun) {
        notify(`Aperçu: ${result.shifts_created} shifts seraient créés`, {
          type: "info",
        });
      } else {
        notify(`✅ ${result.shifts_created} shifts créés avec succès!`, {
          type: "success",
        });
    ...
    try {
      ...
      if (data.employees && !data.groups) {
        setCalendarData((prev: any) => {
          ...
          const reorderedEmployees = data.employees
            .map((emp: any) => {
              const fullEmployee = prev.employees.find(
                (e: any) => e.employee_id === emp.id,
              );
      ...
      if (filterHoursStatus !== "all") {
        const utilization =
          employee.max_monthly_hours > 0
            ...
            : 0;
        if (filterHoursStatus === "over_limit" && !employee.hours_exceeded)
          return false;
        if (filterHoursStatus === "under_50" && utilization >= 50) return false;
        if (
          filterHoursStatus === "ok" &&
          (employee.hours_exceeded || utilization < 50)
        )
          return false;
      }
  ...
  const isToday = (day: number) => {
    ...
    return (
      planning.year === today.getFullYear() &&
  ...
  return (
    <Box>
      ...
      >
        <Box display="flex" alignItems="center" gap={2}>
          ...
          {planning.last_optimized_at && (
            <Tooltip
              title={`Algorithme: ${planning.last_optimization_algorithm || "CP-SAT"} | Durée: ${planning.last_optimization_duration?.toFixed(1) || "?"}s`}
            >
              <Chip
        ...
        <Box display="flex" gap={1}>
          ...
          />
          <Tooltip
            title={!isDraft ? "Disponible uniquement en mode Brouillon" : ""}
          >
            <span>
              <Button
          ...
          <Tooltip
            title={
              stickyEmployeeColumn
                ? "Libérer colonne Employé"
                : "Fixer colonne Employé"
            }
          >
            <IconButton
              size="small"
      ...
      <div
        className={
          stickyEmployeeColumn ? "planning-table-container" : undefined
        }
        style={
          !stickyEmployeeColumn
            ? {
          ...
          <TableHead>
            <TableRow>
              ...
              {prevWeekDays.map((prevDay) => {
                ...
                const isLastPrevDay =
                  prevDay === prevWeekDays[prevWeekDays.length - 1];
                return (
                  <TableCell
                    key={`prev-${prevDay}`}
                    align="center"
                  ...
                  >
                    <Box>
                      ...
                      >
                        ...
                        >
                          👨‍⚕️ {staffCount.infirmier} | 🤝{" "}
                          {staffCount.aide_soignant} | 🎓 {staffCount.stagiaire}
                        </Typography>
                      </Box>
                    </Box>
                  </TableCell>
            ...
            {(() => {
              ...
              const renderEmployeeRow = (employee: any) => {
                ...
                if (employee.shifts && typeof employee.shifts === "object") {
                  try {
                    const shiftValues = Object.values(employee.shifts).filter(
                      (shift: any) => shift != null,
                    );
                    totalHours = shiftValues.reduce(
                      (sum: number, shift: any) => {
                        const hours =
                          typeof shift === "object" && shift.hours != null
                            ? Number(shift.hours)
                            : 0;
                    ...
                    console.error(
                      "Error calculating total hours for employee:",
                ...
                totalHours =
                  isNaN(totalHours) || totalHours == null
                    ? 0
                    : Number(totalHours);
                ...
                const isHiddenEmployee = hiddenEmployees.has(
                  employee.employee_id,
                );
                ...
                return (
                  ...
                  >
                        ...
                        <Box display="flex" alignItems="flex-start" gap={1}>
                          ...
                          <Checkbox
                            ...
                            onChange={() =>
                              setSelectedEmployeeId(
                                isSelected ? null : employee.employee_id,
                              )
                            }
                            size="small"
                          ...
                          <Box minWidth={0} flex={1}>
                            ...
                            >
                              ...
                              {employee.max_monthly_hours?.toFixed(1)}h
                            </Typography>
                            {(isInactive ||
                              employee.hours_exceeded ||
                              ...
                              <Box display="flex" flexWrap="wrap" gap={0.3}>
                                ...
                                {employee.consecutive_days_violation && (
                                  <Tooltip
                                    title={`${employee.max_consecutive_days} jours consécutifs - Loi: max 6 jours (44h repos)`}
                                  >
                                    <Chip
                                ...
                                {employee.evening_to_morning_violation && (
                                  <Tooltip
                                    title={`Soir→Matin interdit: ${employee.evening_to_morning_violations?.length} violation(s)`}
                                  >
                                    <Chip
                                      label={`🔴 S→M`}
                                      size="small"
                    ...
                    {prevWeekDays.map((prevDay) => {
                      const prevShift =
                        previous_week?.shifts?.[employee.employee_id]?.[
                          prevDay
                        ];
                      const isLastPrevDay =
                        prevDay === prevWeekDays[prevWeekDays.length - 1];
              ...
              if (groupBy !== "none" && groupedData?.groups) {
                return groupedData.groups.flatMap(
                  (
                    group: { group: string; employees: any[] },
                    groupIndex: number,
                  ) => {
                    // Sort employees within each group by employee_id for consistent ordering
                    const groupEmployees = (group.employees || []).sort(
                      (a: any, b: any) =>
        ...
        <DialogContent>
          <Box display="flex" flexDirection="column" gap={2} mt={2}>
            ...
            {selectedAlgorithm === "CP-SAT" && (
              <Alert severity="info">
                <Typography variant="subtitle2" gutterBottom>
                  <strong>CP-SAT (Constraint Programming)</strong> - Approche
                  standard
                </Typography>
                <Typography variant="body2">
                  Utilise Google OR-Tools avec configuration par défaut. Rapide
                  et fiable pour la plupart des plannings. Temps d'exécution:
                  ~60 secondes.
                </Typography>
              </Alert>
            )}
            ...
            {selectedAlgorithm === "HYBRID" && (
              <Alert severity="success">
                <Typography variant="subtitle2" gutterBottom>
                  <strong>HYBRID (GA + CP-SAT)</strong> - Meilleure qualité
                  (Recommandé)
                </Typography>
                <Typography variant="body2">
                  Combine les deux approches: l'algorithme génétique trouve les
                  meilleurs paramètres, puis CP-SAT génère la solution.
                  Résultats optimaux mais plus lent (~5 minutes).
                </Typography>
                <Typography variant="body2" sx={{ mt: 1 }}>
                  <strong>⚡ Avantages:</strong> Meilleure qualité de solution,
            ...
            {selectedAlgorithm === "GA" && (
              <Alert severity="warning">
                ...
                <Typography variant="body2">
                  Approche purement évolutionnaire. Non recommandé - utilisez
                  HYBRID à la place. Peut violer certaines contraintes.
                </Typography>
              </Alert>
            )}
            ...
            <Alert severity="success">
              ...
              <ul style={{ margin: "8px 0", paddingLeft: "20px" }}>
                ...
                <li>
                  🏥 <strong>Couverture CONSTANTE: 4 soignants/jour</strong>{" "}
                  (TOUS les jours: semaine, weekend, fériés)
                </li>
                <li>
                  👨‍⚕️{" "}
                  <strong>
                    Au moins 1 infirmier(ère) en matin ET 1 en soir
                  </strong>{" "}
                  (chaque jour)
                </li>
                <li>
                  ⏰ Au moins 1 personne en matin ET 1 en soir (chaque jour)
                </li>
                <li>📊 63% matin / 37% soir (ratio mensuel optimal)</li>
                <li>
                  ⚖️ Distribution équitable et équilibrée (max 3:1 matin/soir)
                </li>
              </ul>
            </Alert>

            <Alert severity="warning">
              <strong>Attention:</strong> Cette opération remplacera toutes les
            ...
            <Box display="flex" justifyContent="center" mt={2}>
              ...
              >
                {optimizing
                  ...
                  : `Lancer l'optimisation (${selectedAlgorithm})`}
              </Button>
            </Box>
          </Box>
        </DialogContent>
        ...
        <DialogContent>
          <Grid container spacing={2} sx={{ mt: 1 }}>
            ...
            <Grid item xs={12}>
              ...
              {newShift.start_time &&
                ...
                !shiftValidationError && (
                  <Alert severity="info" sx={{ mb: 1 }}>
                    Calcul:{" "}
                    {(() => {
                      const [startHour, startMin] = newShift.start_time
                        .split(":")
                        .map(Number);
                      const [endHour, endMin] = newShift.end_time
                        .split(":")
                        .map(Number);
                      const startMinutes = startHour * 60 + startMin;
                      let endMinutes = endHour * 60 + endMin;
                      ...
                      const breakHours = (newShift.break_minutes / 60).toFixed(
                        1,
                      );
                      const workedHours = (
                        (totalMinutes - newShift.break_minutes) /
                        60
                      ).toFixed(1);
              ...
              <Alert severity="info">
                Les shifts créés seront disponibles immédiatement dans le
                planning pour tous les employés.
              </Alert>
            </Grid>
          </Grid>
        </DialogContent>
...
BH,2025-12-01,S13.5-22`}
              ...
              <Typography variant="caption" sx={{ display: "block", mb: 1 }}>
                Format automatiquement détecté - employés en lignes, jours en
                colonnes
              </Typography>

              <Typography variant="body2" sx={{ mt: 2 }}>
                ℹ️ <strong>Notes:</strong>
                <br />• Vous pouvez utiliser <strong>abbreviation</strong> (ex:
                AC, BH) ou <strong>employee_id</strong>
                <br />
                • Format de date: YYYY-MM-DD
                <br />
                • Les codes OFF, CP (congés), et "cours" sont ignorés
                <br />• Les shifts importés seront{" "}
                <strong>protégés de l'optimiseur</strong>
              </Typography>
            </Alert>
      ...
      >
        ...
        <DialogContent>
          {analysisData && (
            <Box display="flex" flexDirection="column" gap={3} mt={2}>
                  ...
                  <Grid item xs={6}>
                    <Paper sx={{ p: 2, textAlign: "center" }}>
                      <Typography variant="h5" color="warning.main">
                        {analysisData.current_state.total_hours?.toFixed(1) ||
                          0}
                        h
                      </Typography>
              ...
              {analysisData.unassigned_opportunities?.underutilized_employees
                ?.length > 0 && (
                <Box>
                  <Typography variant="h6" gutterBottom color="warning.main">
                    💡 Opportunités d'amélioration (
                    {
                      analysisData.unassigned_opportunities
                        .underutilized_employees.length
                    }
                    )
                  </Typography>
                  <TableContainer component={Paper} sx={{ maxHeight: 300 }}>
                    <Table size="small" stickyHeader>
                      ...
                      <TableBody>
                        {analysisData.unassigned_opportunities.underutilized_employees.map(
                          (emp: any) => (
                            <TableRow key={emp.employee_id}>
                              <TableCell>
                              ...
                              <TableCell align="right">
                                <Chip
                                  ...
                                  color={
                                    emp.utilization < 80 ? "error" : "warning"
                                  }
                                  size="small"
                                />
                              </TableCell>
                              <TableCell align="right">
                                {emp.current_hours?.toFixed(1) || 0}h /{" "}
                                {emp.max_hours?.toFixed(1) || 0}h
                                <br />
                                <Typography
                                  variant="caption"
                                  color="warning.main"
                                >
                                  +{emp.available_hours?.toFixed(1) || 0}h
                                  disponible
                                </Typography>
                              </TableCell>
                              <TableCell align="right">
              ...
              {analysisData.proposed_changes?.length > 0 && (
                <Box>
                  <Typography variant="h6" gutterBottom color="success.main">
                    ...
                    {analysisData.proposed_changes.length})
                  </Typography>
                  <TableContainer component={Paper} sx={{ maxHeight: 400 }}>
                    <Table size="small" stickyHeader>
                      ...
                      <TableBody>
                        {analysisData.proposed_changes.map(
                    ...
                    {analysisData.improvement_suggestions.map(
                      (suggestion: any, idx: number) => (
                        <Alert
                          ...
                          icon={
                            suggestion.type === "underutilization" ? (
                              <TrendingUpIcon />
                            ) : suggestion.type === "low_coverage" ? (
                              <AssessmentIcon />
                            ) : (
                              <AssessmentIcon />
                            )
              ...
              {(!analysisData.improvement_suggestions ||
                analysisData.improvement_suggestions.length === 0) && (
                <Alert severity="success">
                  ✅ Aucune suggestion d'amélioration - le planning semble
                  optimal!
                </Alert>
              )}
            </Box>
        ...
        <DialogActions>
          <Button onClick={() => setAnalysisDialog(false)}>Fermer</Button>
                ...
                <Table size="small" stickyHeader>
                  ...
                  <TableBody>
                    {validationResults.results?.map((emp: any) => {
                      ...
                      const isExpanded = expandedValidationRows.has(
                        emp.employee_id,
                      );
                      ...
                      return (
                        <React.Fragment key={emp.employee_id}>
                          ...
                          >
                            ...
                            <TableCell align="center">
                              {emp.checks?.consecutive_days?.status ===
                              "error" ? (
                                <Chip
                            ...
                            <TableCell align="center">
                              {emp.checks?.evening_morning?.violations > 0 ? (
                                <Chip
                                  label={
                                    emp.checks?.evening_morning?.violations
                                  }
                                  size="small"
                                  color="error"
                                  icon={<WarningAmberIcon />}
                            ...
                            <TableCell align="center">
                              {emp.checks?.holiday_conflicts?.violations > 0 ? (
                                <Chip
                                  label={
                                    emp.checks?.holiday_conflicts?.violations
                                  }
                                  size="small"
                                  color="warning"
                                  icon={<WarningAmberIcon />}
                                ...
                                >
                                  <Box sx={{ py: 1, px: 2 }}>
                                    <Grid container spacing={2}>
                                      ...
                                      {emp.checks?.consecutive_days?.periods
                                        ?.length > 0 && (
                                        <Grid item xs={12} md={4}>
                                          ...
                                          <Box
                                            ...
                                            sx={{ m: 0, pl: 2 }}
                                          >
                                            {emp.checks.consecutive_days.periods.map(
                                      ...
                                      {emp.checks?.evening_morning?.details
                                        ?.length > 0 && (
                                        <Grid item xs={12} md={4}>
                                          <Typography
                                            variant="subtitle2"
                                          ...
                                          <Box
                                            ...
                                            sx={{ m: 0, pl: 2 }}
                                          >
                                            {emp.checks.evening_morning.details.map(
                                      ...
                                      {emp.checks?.holiday_conflicts?.details
                                        ?.length > 0 && (
                                        <Grid item xs={12} md={4}>
                                          <Typography
                                            variant="subtitle2"
                                          ...
                                          <Box
                                            ...
                                            sx={{ m: 0, pl: 2 }}
                                          >
                                            {emp.checks.holiday_conflicts.details.map(
                                              (detail: any, idx: number) => (
                                                <Typography
                                                  component="li"
                                                  variant="body2"
